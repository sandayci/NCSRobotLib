...
void aerWrite(int cmd, uint32_t address, uint32_t value)
{
        int retval;
        aer_mem_t mem;
        mem.address = address;
...
//uint32_t aerWriteMem(uint32_t value)
{
      aerWrite(AER_WRITE, 0, value);
}

//static uint32_t BGSet(uint32_t value)
uint32_t BGSet(uint32_t value)
{
...
//device-specific code starts here
	...
	if (synapse->type == NONPLASTIC) {
	    ...
	    num_latches = 5;
            ...
            uint32_t latches[2];
            latches[0] = synapse->recurrent; //recurrent
            latches[1] = 1; //broadcast not used at the moment
	    num_latches = 2;
            offset = 1051136;
	}
	else return -1;
